...
import person.domain.gytis.courier.Person;
...
public class AdminJpaEntityPersonQueryServiceTest {
    ...
    void findAllPaginated_shouldReturnPaginatedResults() {
        ...
        List<Person> mockPersonJpaEntities = List.of(user2, user1);
        Page<Person> mockPage = new PageImpl<>(mockPersonJpaEntities, PageQuery.of(0, 2), mockPersonJpaEntities.size());
    ...
    void findAllPaginated_shouldHandleDifferentSortDirections() {
        List<Person> mockPersonJpaEntities = List.of(user1, user2);
        when(personRepository.findAll(ArgumentMatchers.<Specification<Person>>any(), any(Pageable.class))).thenReturn(new PageImpl<>(mockPersonJpaEntities));
